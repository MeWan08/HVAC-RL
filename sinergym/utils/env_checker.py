...
def _check_returned_values(
    ...
    assert (
        len(data) == 5
    ), "The `step()` method must return four values: obs, reward, terminated, truncated, info"
...
def _check_spaces(env: gym.Env) -> None:
    ...
    assert env.has_wrapper_attr('observation_space'), (
        'You must specify an observation space (cf gym.spaces)' + gym_spaces
    )
    assert env.has_wrapper_attr('action_space'), (
    ...
    assert isinstance(env.action_space, spaces.Space), (
        "The action space must inherit from gymnasium.spaces" + gym_spaces
    )
...
#                                 MAIN FUNCTION                                #
# ---------------------------------------------------------------------------- #
def check_env(env: gym.Env, skip_render_check: bool = True) -> None:
    """
